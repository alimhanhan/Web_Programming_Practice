...
public class MemberMgr {
	...
	public Vector<MemberBean> memberList() {
	    ...
	    try {
	        ...
	        while (rs.next()) {
	            ...
	            Vector<String> selectedHobby = new Vector<String>();
	            for (int i = 0; i < hobbys.length; i++) {
	                if (hobby.charAt(i) == '1') {
	                    selectedHobby.add(hobbys[i]);
	                }
	            }
	            bean.setHobby(selectedHobby.toArray(new String[selectedHobby.size()]));
	            bean.setJob(rs.getString("job"));
	            memberList.add(bean);
	        }
	    } catch (Exception e) {
	...
	public boolean deleteMember(String id, String pwd) {
	    ...
	    try {
	        con = pool.getConnection();
	        String sql = "delete from tblMember where id = ? and pwd = ?";
	        pstmt = con.prepareStatement(sql);
	        pstmt.setString(1, id);
	        pstmt.setString(2, pwd);
	        int count = pstmt.executeUpdate();
